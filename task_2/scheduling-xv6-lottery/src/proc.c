...
found:
  ...
  // pstat.tickets[p - ptable.proc] = p->tickets;
  // pstat.ticks[p - ptable.proc] = 0;
...
int cps() ///////////////////////////////////////////////////////////////////
{
  ...
  acquire(&ptable.lock);
  cprintf("---- \t --- \t --- \t ----- \t     -------- \t ------- \t ----- \n");
  cprintf("name \t pid \t used \t state \t     priority \t tickets \t ticks \n");
  cprintf("---- \t --- \t --- \t ----- \t     -------- \t ------- \t ----- \n");
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
  {
    if (p->state == SLEEPING)
    {
      cprintf("%s \t %d \t %d \t SLEEPING \t %d \t %d \t\t %d \n ", p->name, p->pid, p->inuse, p->priority, p->tickets, p->ticks);
    }
